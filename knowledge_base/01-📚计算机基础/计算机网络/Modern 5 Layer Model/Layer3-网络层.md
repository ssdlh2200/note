...
- 删除ARP缓存表：arp -d \*
#### ARP工作流程
<font color="#d83931"><b>ARP协议只能工作在一个局域网(LAN)中</b></font>
ARP协议工作在数据链路层和网络层之间，负责寻找目标主机的MAC地址
![[20251212-02-24-36.svg]]
此时就需要ARP协议来获取主机B的MAC地址，此时
- 主机A会广播一条消息，询问192.168.10.4你的MAC地址是多少？
	- 子网掩码是255.255.255.0，那么广播地址就是192.168.10.255
- 主机B收到该消息后，回复192.168.10.2自己的MAC地址
	- 该回复消息是单播，因为已经知道了主机A的IP和MAC地址
![[20251212-3.png]]
- 主机B收到ARP消息后，会将数据存储到ARP缓存表中
![[20251212-5.png]]
...
| **网络类型**      | **核心工作层次**                    | **主要设备/协议**            |
| ------------- | ----------------------------- | ---------------------- |
| **LAN** (局域网) | **数据链路层 (L2)**                | 交换机、以太网、MAC 地址         |
| **WAN** (广域网) | **网络层 (L3)** 和 **数据链路层 (L2)** | 路由器、MPLS、PPP 等远程 L2 协议 |
一般将光猫转接进来的插入WAN口
## NAT
NAT：（网络地址转换技术）
