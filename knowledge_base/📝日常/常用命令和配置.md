...
- origin：远程仓库默认名
- <仓库URL>
  - ssh:使用ssh拉取推送代码,可以实现免密登录
    - git@github.com:ssdlh2200/note.git
  - HTTPS:需要登录github账号
    - https://github.com/ssdlh2200/note.git

#### 配置ssh免密登录

本地仓库和远程仓库之间传输可以通过ssh加密的，配置验证信息

**本地配置**

1. 生成密钥：ssh-keygen -t rsa -C "2544310147@qq.com"

   - 邮箱可以随便填

   - 第一次enter选择密钥保存位置一般在C:\Users\\<用户名>\\.ssh

   - 第二次enter选择是否给密钥添加一个私钥
     - 使用github一般不添加，否则每次push都会要求输入密码

**远程配置**

2. 查看密钥：cat ~/.ssh/id_rsa.pub

   - 复制密钥，在github的SSH kyes中添加密钥
