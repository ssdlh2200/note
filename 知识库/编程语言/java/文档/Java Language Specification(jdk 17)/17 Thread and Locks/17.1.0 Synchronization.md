...
### 方法
synchronized 方法在调用时会自动锁定对于的monitor
- 实例方法：锁定该方法所属对象的monitor（即this）
- 静态方法：锁定该类的Class对象的monitor
- 无论方法正常执行与否锁都会自动释放
```java
...
```
### 死锁问题、其他同步机制
Java不会阻止也不强制检测死锁（deadlock），线程同时持有多个对象的锁，要使用常规方法避免死锁
synchronized以外，Java还有其他同步机制
- volatile 变量读写
- Lock、Semaphore等