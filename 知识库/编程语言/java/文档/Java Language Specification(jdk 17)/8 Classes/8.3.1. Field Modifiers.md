...
2：inc %r8d
3：mov %r8d,0x70(%r11)
4：lock addl $0x0,-0x40(%rsp) //不会改变任何内容，只为触发缓存一致性，将数据刷新到内存
```
在这里我们能看出来当多线程同时竞争执行的时候，1、2、3号指令仍然可能产生竞态
...
lock指令（出发缓存一致性）
```
从上面的汇编伪代码中可以看出，i 的更新一定在 j 之前对其他线程可见；
- 所以 j 不可能比 i 更新得更早；
- 因此不会出现“j 大于 i”的错误。
⚠️ 但也要注意：
- two() 可能会在读取 i 和 j 之间被其他线程多次修改它们。
- 所以 two() 可能看到的情况是：j 的值远远大于 i（因为在读取 i 后、读取 j 前，one() 已经多次执行）。