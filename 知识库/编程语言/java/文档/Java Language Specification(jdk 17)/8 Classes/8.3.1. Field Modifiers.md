...
- 对于volatile写操作，JMM要求插入 StoreStore Barrier 和 StoreLoadBarrier
- 对于volatile读操作，JMM要求插入 LoadLoad Barrier 和 LoadStoreBarrier
```
普通写（data = 42）
    ↓
[storestore 屏障] //保证上面先执行完
volatile写（ready = true）
    ↓
[storeLoad屏障] //防止写后被乱序到volatile前
```
